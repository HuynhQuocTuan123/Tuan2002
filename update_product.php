...
echo "<SELECT name ='CategoryList' class='from-control'>
	...
	{
		$id = $_GET['id'];
		$sqlString = "SELECT product_name, price, smalldesc, detaildesc, pro_qty, pro_image, cat_id, branch_id from product where product_id='$id'";
	...
	if(isset($_POST['btnUpdate']))
	{
		...
		else
		{
			...
			else
			{
					...
					$sqlString = "UPDATE product set product_name ='$proname', price = '$price', smalldesc ='$short',  detaildesc ='$detail', pro_qty='$qty', cat_id='$cat',branch_id= '$branch' where product_id ='$id'";
					pg_query($conn,$sqlString);
					echo '<meta http-equiv="refresh" content="0;URL =?page=pm"';	
